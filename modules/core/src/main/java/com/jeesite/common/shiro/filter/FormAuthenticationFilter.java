...
public class FormAuthenticationFilter extends org.apache.shiro.web.filter.authc.FormAuthenticationFilter {
	...
	protected AuthenticationToken createToken(ServletRequest request, ServletResponse response) {
		...
		/*
		 * HttpServletRequest hrt = (HttpServletRequest) request; MyRequestWrapper
		 * requestWrapper = null; try { requestWrapper = new MyRequestWrapper(hrt); }
		 * catch (IOException e) { e.printStackTrace(); }
		 * System.out.println(requestWrapper.getBody());
		 * 
		 * String username = ""; String password = ""; boolean rememberMe = false;
		 * List<Map<String, Object>> info =
		 * JsonMapper.fromJsonForMapList(requestWrapper.getBody()); if(info.size()>0) {
		 * Map um = info.get(0); username = (String) um.get("username"); password =
		 * (String) um.get("password"); rememberMe = (boolean) um.get("rememberMe"); }
		 */
