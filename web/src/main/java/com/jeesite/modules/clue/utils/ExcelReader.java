...
public class ExcelReader {
     ...
     */
    public static List<ClueExcelVo> readExcel(MultipartFile file) {
        ...
        try {
            ...
            // 读取excel中的数据
            List<ClueExcelVo> resultDataList = parseExcel(workbook);
     ...
     * @return 读取结果列表，读取失败时返回null
     */
    public static List<ClueExcelVo> readExcel(String fileName) {
...
//            }
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                ClueExcelVo resultData = convertRowToData(row);
                if (null == resultData) {
...
//    }
     ...
     */
    private static ClueExcelVo convertRowToData(Row row) {
    	ClueExcelVo resultData = new ClueExcelVo();
        ...
        String phoneC = df.format(phone);
        resultData.setPhoneNumber(phoneC);
        // 获取年龄
        cell = row.getCell(cellNum++);
        Double ageStr = cell.getNumericCellValue();
        String age = df.format(ageStr);
        if (null == ageStr || "".equals(ageStr)) {
            // 年龄为空
            resultData.setAge(0);
        } else {
            resultData.setAge(Integer.parseInt(age));
        }
        // 获取性别
        cell = row.getCell(cellNum++);
